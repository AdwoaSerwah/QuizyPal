...
def add_choices(data: List[Dict[str, Any]], storage) -> Tuple:
    ...
    Args:
        data (list): A list of dictionaries, each representing a choice. Expected keys:
    ...
    for choice_text in choice_texts:
        if choice_text in existing_choice_texts:
            abort(400, description=f"The choice '{choice_text}' already exists in this question!")
    ...
    validate_correct_answers(all_choices, False, allow_multiple_answers, question.question_text)
    ...
    for choice_data in choices_data:
        ...
        # formatted_text = validate_choice_text(choice_text)

        # Check for duplicate choice text in the same question
        # existing_choice = storage.filter_by(Choice, question_id=question_id, choice_text=formatted_text)
        #if existing_choice:
            # abort(400, description=f"The choice '{formatted_text}' already exists in this question!")
...
def update_choice_by_id(data: Dict[str, Any], storage: Any, choice_id: str) -> tuple:
    """"""
    ...
    if 'choice_text' in data:
        ...
        existing_choice = storage.filter_by(Choice, question_id=question_id, choice_text=choice_text)
        if existing_choice and choice.choice_text != choice_text:
            abort(404, description=f"The choice '{choice_text}' already exists in this question!")
    ...
    if 'is_correct' in data:
        ...
        if is_correct != choice.is_correct:
            ...
            if existing_choices:
                existing_choices = [ch for ch in existing_choices if ch.id != choice.id]
            # Validate the correct answers based on question settings
            validate_correct_answers(existing_choices, is_correct, allow_multiple_answers, question.question_text)
...
def validate_correct_answers(existing_choices, is_correct, allow_multiple_answers, question_text):
    """
    Helper function to validate the number of correct answers based on the question's settings.

    Args:
        existing_choices (list): List of existing choice objects or dictionaries.
        is_correct (bool): Whether the new choice is correct.
        allow_multiple_answers (bool): Whether multiple correct answers are allowed.
        question_text (str): The text of the question to be used in error messages.

    Returns:
        None: Calls abort if validation fails.
    """
    # Count the current correct answers
    # correct_answers_count = sum(1 for choice in existing_choices if choice.is_correct)
    # Count the current correct answers
    ...
    if allow_multiple_answers:
        if correct_answers_count < 1:
            abort(400, description=f"At least one correct choice is required for question '{question_text}'.")
    else:
        if correct_answers_count != 1:
            abort(400, description=f"Exactly one correct choice is required for question '{question_text}'.")
...
def validate_is_correct(is_correct):
    ...
    if not isinstance(is_correct, bool):
        abort(400, description=f"is_correct '{is_correct}' must be a boolean value!")


def ensure_no_answer_choice(question_id: str, storage) -> None:
    """
