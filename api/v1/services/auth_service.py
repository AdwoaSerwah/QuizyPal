#!/usr/bin/env python3
"""
This module contains a custom decorator function, `admin_required`, which ensures
that a user has 'admin' privileges before allowing access to the decorated route.

It uses JWT tokens to verify the role of the user and restricts access to the route
...
def admin_required(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        jwt_data = get_jwt()  # Retrieve the JWT claims
        user_role = jwt_data.get('role')  # Get the 'role' claim from the token
