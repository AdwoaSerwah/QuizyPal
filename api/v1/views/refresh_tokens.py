...
def get_refresh_tokens() -> ResponseReturnValue:
    ...
    result = get_paginated_data(storage, RefreshToken, page=page, page_size=page_size)
...
@app_views.route('/refresh-tokens/<refresh_token_id>', methods=['GET'], strict_slashes=False)
@jwt_required()
def get_refresh_token(refresh_token_id: str = None) -> ResponseReturnValue:
    ...
    This route retrieves a single refresh token based on the provided refresh_token_id.
    ...
    """
    # Call the helper function `get_refresh_token_by_id` to retrieve the refresh token by its ID.
    refresh_token = get_refresh_token_by_id(refresh_token_id, storage)

    # If the refresh token is not found, abort with a 404 error and message "refresh token not found".
    ...
    # Check if the current user is an admin or if they are trying to retrieve their own refresh token
    if current_user_role != "admin" and refresh_token.user_id != current_user_id:
        abort(403, description="You are not authorized to retrieve this refresh token.")
...
def refresh_token() -> ResponseReturnValue:
    ...
    if stored_refresh_token:
        ...
        if not db_refresh_token:
            abort(404, description="Refresh token not found")  # Separate error for non-existent token

        if db_refresh_token.is_expired or db_refresh_token.has_expired():
            abort(401, description="Invalid, revoked, or expired refresh token")

        # if not db_refresh_token or db_refresh_token.is_expired or db_refresh_token.has_expired():
    ...
    redis_client.setex(redis_key, int(refresh_token_exp.total_seconds()), new_refresh_token)
    ...
    if db_refresh_token:
        ...
        db_refresh_token.expires_at = datetime.now(timezone.utc) + refresh_token_exp
        db_refresh_token.is_expired = datetime.now(timezone.utc) > db_refresh_token.expires_at  # Update is_expired
        storage.save()
    else:
        expires_at = datetime.now(timezone.utc) + refresh_token_exp  # Calculate expiration in advance
        db_refresh_token = RefreshToken(
            user_id=current_user,
            token=new_refresh_token,
            expires_at=expires_at,
            is_expired=datetime.now(timezone.utc) > expires_at  # Same logic as the if block
        )
...
def update_refresh_token(refresh_token_id: str) -> ResponseReturnValue:
    ...
    if 'is_revoked' not in data:
        abort(400, description="The 'is_revoked' field is required in the request body to update the refresh token status.")
