...
This module defines the API routes for managing questions in the
QuizyPal application. It provides endpoints for retrieving, creating,
updating, and deleting questions. Some endpoints require admin privileges,
while others are accessible to authenticated users.

Routes:
    - GET /questions: Retrieves all questions with pagination (admin-only).
    - GET /questions/<question_id>: Retrieves a specific question by ID.
    - GET /questions/<question_id>/choices: Retrieves all choices for question,
      ensuring the user has an in-progress quiz (authenticated users).
    - DELETE /questions/<question_id>: Deletes a question by ID (admin-only).
    - POST /questions: Creates a new question (admin-only).
    - PUT /questions/<question_id>: Updates an existing question (admin-only).

Authentication:
    - JWT authentication is required for all endpoints.
    - Admin access is required for question management operations.

Returns:
    - JSON responses with appropriate status codes and messages.
"""
...
from api.v1.services.question_service import (
    get_question_by_id, add_question,
    update_question_by_id, get_choices_for_question
)
...
def get_questions() -> ResponseReturnValue:
    ...
    result = get_paginated_data(storage, Question,
...
def get_question(question_id: str = None) -> ResponseReturnValue:
    ...
    """
    # Call the helper function to retrieve the question by its ID.
    question = get_question_by_id(question_id, storage)

    # If the question is not found, abort with a 404 error.
    if question is None:
...
@app_views.route('/questions/<question_id>/choices',
                 methods=['GET'], strict_slashes=False)
@jwt_required()
def get_question_choices(question_id: str = None) -> ResponseReturnValue:
    """
    Get all choices for a specific question, ensuring the user
    has an in-progress quiz and the question belongs to the associated quiz.
    ...
    # Check if the user has an in-progress result for the quiz
    ...
    if not result and current_user_role != "admin":
        abort(403, description=(
            "You do not have an in-progress quiz for the associated quiz."
        ))
...
def delete_question(question_id: str = None) -> ResponseReturnValue:
    ...
    This route deletes a question after verifying the identity
    of the question making the request.
    ...
    Return:
        ...
        If the question does not exist or the current question is unauthorized,
        it returns an error.
    """
    question = get_question_by_id(question_id, storage)
...
def create_question() -> ResponseReturnValue:
    ...
    This route allows admins to create a new question by accepting
    the necessary information in a JSON payload. The input is validated,
    and duplicate questions or invalid data are rejected.

    Return:
        A JSON response with the created question object or error messages
        for invalid input.
    """
...
def update_question(question_id: str = None) -> ResponseReturnValue:
    ...
    This route allows admins to update an existing question
    by providing updated information in a JSON payload. Validations
    ensure updates are consistent with the business rules.

    URL Params:
        - question_id (str): The ID of the question to update.

    Returns:
        A JSON response with the updated question object or error messages
        for invalid input.
    """
    # Ensure request data is JSON
    if not request.get_json():
